...
void TrainingWriteBuffers::addRow(
  ...
  if (!data.hasFullData)
  {
    cout << "TrainingWriteBuffers::addRow has not full data, maybe because vcf hits the node limit\n";
    return;
  }
  ...
  //还剩几手终局
  if (data.endHist.isGameFinished && (data.endHist.isNoResult || data.endHist.winner == C_EMPTY)) rowGlobal[22] = boardArea - whiteValueTargetsIdx;
  else rowGlobal[22] = whiteValueTargets.size() - whiteValueTargetsIdx;
