...
vector<NNEvaluator*> Setup::initializeNNEvaluators(
  ...
  if (cfg.contains("maxVCFnodes"))
    VCFsolver::MAXNODE = cfg.getInt("maxVCFnodes" ,10, 100000000);
...
vector<SearchParams> Setup::loadParams(
  ...
  for(int i = 0; i<numBots; i++) {
    ...
    if(cfg.contains("cpuctExploration"+idxStr)) params.cpuctExploration = cfg.getDouble("cpuctExploration"+idxStr, -10.0, 10.0);
    else if(cfg.contains("cpuctExploration"))   params.cpuctExploration = cfg.getDouble("cpuctExploration",        -10.0, 10.0);
    ...
    if (cfg.contains("useVCF" + idxStr)) params.useVCF = cfg.getBool("useVCF" + idxStr);
    else if (cfg.contains("useVCF"))   params.useVCF = cfg.getBool("useVCF");
    else                               params.useVCF = true;


    if (cfg.contains("finishMovenumFactor" + idxStr)) params.finishMovenumFactor = cfg.getDouble("finishMovenumFactor" + idxStr, -0.4, 1.0);
    else if (cfg.contains("finishMovenumFactor"))   params.finishMovenumFactor = cfg.getDouble("finishMovenumFactor", -0.4, 1.0);
    else params.finishMovenumFactor = 0.0;

    //nbest is not in CFG file
