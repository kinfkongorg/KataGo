...
bool Search::getNodeRawNNValues(const SearchNode& node, ReportedSearchValues& values) const {
  ...
  values.staticScoreValue = 0;
  values.dynamicScoreValue = 0;
...
AnalysisData Search::getAnalysisDataOfSingleChild(
  ...
  if(child != NULL) {
    ...
    expectMovenumSum = child->stats.varTimeSum;
    child->statsLock.clear(std::memory_order_release);
  }
  ...
  if(weightSum <= 1e-30) {
    ...
    data.noResultValue = parentNoResultValue;
    data.ess = 0.0;
    data.expectFinishGameMovenum = 0;
  }
  else {
    ...
    data.lead = lead;
    data.noResultValue = noResultValue;
    data.ess = weightSum * weightSum / weightSqSum;
    data.expectFinishGameMovenum = expectMovenumSum / weightSum;
  }
...
void Search::getAnalysisData(
  ...
  double parentLead;
  {
    ...
    parentNoResultValue = noResultValueSum / weightSum;
  ...
  for(int i = 0; i<numChildren; i++) {
    ...
    AnalysisData data = getAnalysisDataOfSingleChild(
      ...
      parentScoreMean, parentScoreStdev, parentLead, parentNoResultValue, maxPVDepth
    );
...
void Search::printTree(ostream& out, const SearchNode* node, PrintTreeOptions options, Player perspective) const {
  ...
  AnalysisData data;
  {
    ...
    data = getAnalysisDataOfSingleChild(
      ...
      parentScoreMean, parentScoreStdev, parentLead, parentNoResultValue,options.maxPVDepth_
    );
